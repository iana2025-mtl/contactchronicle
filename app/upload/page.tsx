  ...
  return (
    <ProtectedRoute>
      ...
      <main className="flex-1 container mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6 lg:py-8">
        <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold mb-4 sm:mb-6 text-purple-700">Upload Contacts</h1>
        ...
        <div className="bg-gradient-to-r from-blue-100 to-purple-100 border border-purple-200 rounded-lg p-3 sm:p-4 mb-4 sm:mb-6 shadow-sm">
          <h2 className="text-xs sm:text-sm font-semibold text-purple-700 mb-2">📋 Supported File Formats:</h2>
        ...
        <div
          ...
          className={`border-2 border-dashed rounded-lg p-6 sm:p-8 lg:p-12 text-center mb-4 sm:mb-6 transition-colors ${
            isDragging
              ? 'border-purple-300 bg-purple-100 shadow-lg'
        ...
        >
          <input
            ...
            className="hidden"
          />
          <p className="text-base sm:text-lg text-purple-600 mb-2">
        ...
        {csvData.length > 0 && (
          <div className="bg-white rounded-lg shadow-sm border border-purple-200 p-4 sm:p-6 mb-4 sm:mb-6 bg-gradient-to-br from-white to-blue-50">
            <h2 className="text-lg sm:text-xl font-semibold mb-4 text-purple-700">Prompt to Confirm Key Columns:</h2>
            <div className="space-y-3 sm:space-y-4">
              {expectedColumns.map((expected) => (
                <div key={expected} className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                  <label className="w-full sm:w-48 lg:w-64 text-sm font-medium text-purple-600">
                    {expected}:
                  </label>
                  <select
                    value={columnMapping[expected] || ''}
                    onChange={(e) => handleColumnMappingChange(expected, e.target.value)}
                    className="flex-1 px-3 sm:px-4 py-2.5 sm:py-2 text-base border border-purple-200 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-300 bg-white"
                  >
        ...
        {previewData.length > 0 && (
          <div className="bg-white rounded-lg shadow-sm border border-purple-200 p-6 mb-6 bg-gradient-to-br from-white to-purple-50">
            <h2 className="text-xl font-semibold mb-2 text-purple-700">Preview Records</h2>
            <p className="text-sm text-purple-500 mb-4">Table View of the Data Uploaded</p>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-gradient-to-r from-purple-200 to-pink-200 border-b border-purple-200">
                  <tr>
                    <th className="px-4 py-2 text-left font-medium text-purple-700">First Name</th>
                    <th className="px-4 py-2 text-left font-medium text-purple-700">Last Name</th>
                    <th className="px-4 py-2 text-left font-medium text-purple-700">Email</th>
                    <th className="px-4 py-2 text-left font-medium text-purple-700">Phone</th>
                    <th className="px-4 py-2 text-left font-medium text-purple-700">Source</th>
                  </tr>
                </thead>
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
          <button
            ...
            className="flex-1 sm:flex-initial px-4 sm:px-6 py-3 bg-gradient-to-r from-purple-300 to-pink-300 text-purple-800 rounded-lg hover:from-purple-400 hover:to-pink-400 transition-all font-medium shadow-sm text-sm sm:text-base"
          >
            Confirm
          </button>
          <button
            onClick={handleCancel}
            className="flex-1 sm:flex-initial px-4 sm:px-6 py-3 bg-pink-200 text-pink-600 rounded-lg hover:bg-pink-300 transition-colors font-medium shadow-sm text-sm sm:text-base"
          >
            Cancel
          </button>
        </div>
